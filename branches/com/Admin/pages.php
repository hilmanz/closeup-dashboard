		...
		if( count($err) > 0 ){
			...
			$qry = "INSERT INTO gm_pages
						...
						('".mysql_escape_string(strtolower($req->getPost('request')))."',
						 '".mysql_escape_string($req->getPost('name'))."',
						 'static',
						 '".mysql_escape_string($req->getPost('template'))."',
						 '".mysql_escape_string($req->getPost('status'))."',
						 '".mysql_escape_string($req->getPost('content'))."',
			...
			if($this->query($qry)){
				...
				//if( $req->getPost('content') == 'yes' ){
					...
					$qry = "INSERT INTO gm_page_content
								...
								VALUES
								('".$page_id."','".mysql_escape_string($req->getPost('content_title'))."','".mysql_escape_string($req->getPost('content_text'))."')";
	...
	function editPage($id,$req,$name,$request,$content){
		...
		if( count($err) > 0 ){
			...
			$qry = "UPDATE gm_pages SET
						...
						page_name='".mysql_escape_string($req->getPost('name'))."',
						page_template='".mysql_escape_string($req->getPost('template'))."',
						page_status='".mysql_escape_string($req->getPost('status'))."',
						page_content='".mysql_escape_string($req->getPost('content'))."',
			...
			if($this->query($qry)){
				...
				//if($content == 'yes'){
					...
					$qry = "UPDATE gm_page_content SET
								content_title='".mysql_escape_string($req->getPost('content_title'))."',
								content_text='".mysql_escape_string($req->getPost('content_text'))."'
								WHERE
								page_id='".$id."'";
